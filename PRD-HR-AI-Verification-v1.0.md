...
|---|---|---|---|---|
| **김철수** (공공기관 PM, 38세) | 연간 5,000명 공채 관리 | 감사 지적 시 수기 캡처 증거 無 → 징계 리스크 | 병렬 캡처 기반 면책권 리포트 | §3-Story-1 |
| **박지민** (채용 대행사 팀장, 42세) | 수익률 1% 저마진 운영 | 인건비 비중 과다 + 클레임 폭증 | 인건비 50% 절감 + 오류율 0 | §3-Story-2 |
| **이현우** (핀테크 HRBP, 35세) | 수시 대규모 경력직 채용 | 4대보험 이력 위조 미검출 + 민원 전화 70% | Triple Check Loop + 공수 0 UX | §3-Story-3 |
...
| **홍길동** (긴급 채용 PM, 50세) | 3일 내 1,000명 검증 | 저화질 이미지 인식 실패 + 시간 無 | 저화질 보정 OCR + 대량 배치 처리 | §3-Story-3 |
...
### Story-3 | Self-Service 알림톡 루프

**Story:**
> As a **채용 대행사 팀장 (박지민)**, I want **서류 불일치 또는 미비 감지 즉시 지원자에게 카카오알림톡이 자동 발송되고, 지원자가 스스로 재제출·수정할 수 있는 링크**를 받고 싶다, so that **민원 전화 응대 업무 90% 이상을 제거하고 실무 수익성을 회복**할 수 있다.

**AC-1 | 불일치 감지 후 알림톡 발송 속도**
- **Given** Triple Check Loop 실행 결과 '불일치' 또는 '서류 미비' 상태가 확정된 상태에서
- **When** 검증 엔진이 결과를 저장하면
- **Then** 해당 지원자의 등록 연락처로 카카오알림톡이 **3분 이내** 자동 발송되어야 한다.
  - 임계치: 알림톡 발송 성공률 **≥ 99%** / 발송 지연(> 3분) 건에 대한 재시도 1회 + 실패 시 SMS 폴백
  - 실패 케이스: 지원자 연락처 미등록 → 담당자 대시보드에 '연락처 없음' 플래그 및 수동 안내 요청 표시

**AC-2 | 재제출 링크 유효성 및 완료율**
- **Given** 지원자가 알림톡 내 재제출 링크를 수신한 상태에서
...
**AC-3 | 민원 전화 감소 측정 가능성**
- **Given** Self-Service 루프가 활성화된 채용 회차에서
- **When** 해당 회차가 종료되면
- **Then** 시스템이 '알림톡 발송 건수 / 재제출 완료 건수 / 담당자 수동 개입 건수'를 회차별로 자동 집계하여 대시보드에 표시해야 한다.
  - 임계치: Self-Service 처리율 **≥ 90%** (수동 개입 건 ≤ 10%) / 리포트 생성 **≤ 10초**
  - 실패 케이스: 알림톡 발송 후 72시간 내 미응답 → 담당자에게 '미응답 지원자 목록' 자동 전송
...
graph LR
    ...
    C --> E[대시보드 지표 업데이트]
    D --> F{임계치 초과?}
    F -- Yes --> G[🚨 PagerDuty 알림<br>· 오류율 > 0.5%<br>· 응답시간 p95 > 5s<br>· 캡처 실패율 > 0.5%]
    F -- No --> H[일반 로그 집계]
    G --> I[온콜 엔지니어 대응]
```
...
- 원본-조회본 병렬 캡처 + 타임스탬프 기반 Audit Trail 생성
...
- Self-Service 알림톡 루프, 외부 API 연동 (Should/Could 항목 → Phase 2)
