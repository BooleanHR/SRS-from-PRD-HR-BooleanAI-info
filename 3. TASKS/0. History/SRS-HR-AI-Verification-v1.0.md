# HR AI 서류 진위확인 솔루션
# Software Requirements Specification (SRS) v0.3 — MVP / PoC 기준
**Document ID:** SRS-001  
**Revision:** 0.3  
**Date:** 2026-04-16  
**Standard:** ISO/IEC/IEEE 29148:2018  
**Status:** MVP PoC 기준 재조정 완료  
**Tech Stack:** Next.js (App Router) · Supabase (PostgreSQL) · Prisma · Gemini API (Vercel AI SDK) · Tailwind CSS · shadcn/ui · Vercel  
...
> ## ⚠️ v0.3 변경 핵심 원칙
>
> 본 문서는 **"입문자 수준 개발자가 바이브코딩으로 실제 구현 가능한 MVP"** 를 기준으로 재작성되었습니다.
>
> **제거된 것 (v0.2 → v0.3):** Inngest, QStash, Browserless.io, Supabase Realtime, WORM 스토리지 정책, ORGANIZATION 멀티테넌시, 배치 처리 스케줄러(4,000건/시간 SLA), REST API 외부 연동/SDK/개발자 포털, False Negative 샘플링 배치, ISMS-P/CSAP 인증 요건
>
> **추가된 것:** C-TEC 스택 Constraints, PoC 증명 요소 명시, Mock/Dummy 대체 목록, 로컬(SQLite) → 배포(Supabase) 전략
...
| 병렬 캡처 이미지 (Browserless.io) | Vercel Serverless에서 Headless Browser 실행 불가 | Placeholder 이미지 URL 저장 + 실제 원본 파일 경로 대체 |
...
| REF-ID | 문서명 | 비고 |
|---|---|---|
| REF-01 | PRD-HR-AI-Verification-v0.1 | 비즈니스/기능 요구 원천 |
...
### Product Functions Summary (MVP PoC)
...
| Must | F3 | Audit Trail DB 기록 | Prisma → Supabase PostgreSQL |
...
| Should | F7 | 불일치 알림 (이메일) | Resend API (Route Handler) |
| Should | F8 | 이미지 전처리 | Gemini Vision 내장 처리 |
| Could | F9 | 대시보드 통계 | Prisma 집계 쿼리 |
...
| 422 | Unprocessable | 파일 크기 초과 (20MB) |
| 500 | Internal Error | 서버 내부 오류 |

---

## 3.8 ERD — Prisma + Supabase PostgreSQL

> **PoC 범위:** ORGANIZATION 멀티테넌시 제거. 단일 조직 기준 운영. USER, APPLICANT, DOCUMENT, VERIFICATION_JOB, AUDIT_TRAIL, NOTIFICATION, BATCH, REPORT 총 8개 엔터티.
...
model AuditTrail {
  ...
  captureFilePath   String?   @map("capture_file_path") // 원본 파일 경로 (Mock 시 null 허용)
...
model Notification {
  ...
  status        String    // SENT | FAILED | PENDING | NO_CONTACT
...
## 3.9 Sequence Diagrams (MVP 단순화 흐름)

### Seq-01 | 서류 업로드 → Triple Check Loop → Audit Trail (핵심 흐름)
...
sequenceDiagram
    ...
    User->>UI: 서류 파일 업로드 (PDF/JPG/PNG)
    UI->>SA: uploadAndVerify(formData) Server Action 호출

    SA->>ST: 원본 파일 저장 → raw_file_path 반환
    SA->>DB: DOCUMENT 레코드 생성 (status=PENDING)

    SA->>GEM: Gemini Vision API 호출 (이미지 + 추출 프롬프트)
    Note over GEM: generateObject()로 구조화 JSON 반환<br/>(성명, 발급번호, 발급일자, 발급기관명, confidence_score)
    GEM-->>SA: { fields: {...}, confidence_score: 85 }

    SA->>DB: DOCUMENT.ocr_extracted_json, ocr_confidence_score 저장

    alt OCR 신뢰도 < 70 (PoC 임계치)
        SA->>DB: VERIFICATION_JOB status = MANUAL_REVIEW
        ...
        SA->>DB: VERIFICATION_JOB 저장 (status, discrepancy_detail, confidence_score)

        SA->>DB: AUDIT_TRAIL 생성 (timestamp_hash, is_immutable=true, capture_file_path)
        Note over DB: capture_file_path = 원본 파일 경로<br/>(병렬 캡처 Mock → 향후 실제 구현)

        alt FAIL (불일치)
            SA->>SA: sendNotificationEmail() → Resend API
            Note over SA: 카카오 알림톡 대신 이메일로 대체
        end

        SA-->>UI: { job_id, status, confidence_score, discrepancy_detail }
    end
...
sequenceDiagram
    ...
    Manager->>UI: 상세 화면 클릭
    UI-->>Manager: OCR 추출값 vs 기관 DB 반환값 비교 표시<br/>(불일치 항목 하이라이트)
...
### Seq-03 | PDF 리포트 생성 (동기, 100건 이하 PoC 기준)
...
sequenceDiagram
    ...
    Note over PDF: 회차명, 검증일시, 건별 PASS/FAIL 결과,<br/>원본 파일 경로 포함 (병렬 캡처 이미지 미포함 → Phase 2)

    PDF-->>SA: PDF Buffer
    SA->>DB: REPORT 레코드 생성 (status=COMPLETED)
    SA-->>UI: PDF Buffer 반환
...
| 컴포넌트 | 역할 | 비고 |
|---|---|---|
| Next.js Pages | 서류 업로드 UI, 대시보드, 승인/반려 화면 | shadcn/ui 컴포넌트 사용 |
| Server Actions | OCR 요청, Triple Check, Audit Trail 생성, 승인/반려 | `"use server"` 함수 |
| Route Handlers | SWR polling 응답, PDF 스트림 반환 | `app/api/` 경로 |
...
graph LR
    ...
    subgraph System["HR AI 서류 진위확인 솔루션 (MVP)"]
    ...
    A1 --> UC01 & UC04 & UC05
    A2 --> UC01 & UC04 & UC05 & UC06 & UC07 & UC09
    A3 --> UC07 & UC08
...
classDiagram
    ...
    class AuditTrailService {
        <<Server Action>>
        +createTrail(jobId, filePath) AuditTrail
    ...
    class NotificationService {
        <<Server Action>>
        +sendEmail(applicantEmail, jobId) void
        +logDashboardAlert(jobId) void
        -buildEmailContent(job) String
    }
    ...
    VerificationService --> NotificationService : 불일치 알림
...
## 3.13 Specific Requirements — Functional Requirements
...
| REQ-FUNC-042 | PDF에는 회차명, 검증일시, 건별 결과, `mock_used` 표시(Mock 데이터 사용 건 별도 표기)가 포함되어야 한다. | Must | **Given** PDF 생성 완료 **When** 내용 확인 **Then** 4개 항목 포함; mock_used=true 건에 '(Mock)' 표기 |

### F6 — 불일치 이메일 알림 (Should)

| ID | 요구사항 | Priority | AC |
|---|---|---|---|
| REQ-FUNC-050 | Triple Check Loop 결과 FAIL 확정 시 Resend API로 담당자 이메일 발송. 발송 실패 시 대시보드 알림 텍스트로 대체. | Should | **Given** FAIL 확정 **When** Server Action 실행 **Then** 이메일 발송 또는 대시보드 알림 표시 |
| REQ-FUNC-051 | 이메일에는 지원자 이름(마스킹), 서류 유형, 불일치 항목 요약, 대시보드 링크가 포함되어야 한다. | Should | **Given** FAIL 이메일 **When** 이메일 수신 **Then** 4개 항목 포함 확인 |

### F7 — 대시보드 통계 (Could)

| ID | 요구사항 | Priority | AC |
|---|---|---|---|
| REQ-FUNC-060 | 대시보드에 총 검증 건수, PASS/FAIL/MANUAL_REVIEW 건수, Mock 사용 건수가 표시되어야 한다. | Could | **Given** 검증 완료 건 존재 **When** 대시보드 접속 **Then** 5개 지표 표시 |
...
| REQ-NF-012 | Mock 응답 투명성 | mock_used=true 건 100% 표시 | PDF 리포트 및 대시보드 |
...
## 3.15 Traceability Matrix (RTM) — MVP
...
## 3.17 Validation Plan (PoC 검증 계획)

| 실험 ID | 검증 항목 | 방법 | 성공 기준 |
|---|---|---|---|
| Exp-1 | Gemini Vision OCR 정확도 | 실제 학위증명서 10건 수동 입력값 vs OCR 비교 | 핵심 필드 정확도 ≥ 80% |
| Exp-2 | Triple Check FAIL 감지 | 의도적 불일치 데이터 5건 투입 | 5건 모두 FAIL + discrepancy_detail 정확 |
| Exp-3 | Audit Trail 불변성 | 직접 DELETE API 호출 시도 | HTTP 403 반환 + 레코드 유지 |
| Exp-4 | PDF 리포트 완성도 | 10건 배치 PDF 생성 후 내용 확인 | 필수 항목 누락 0건 |
| Exp-5 | E2E 흐름 데모 | 서류 업로드 → 검증 → 승인 → PDF 다운로드 | 전체 흐름 오류 없이 완료 |

---

*— End of SRS-001 v0.3 (MVP PoC 기준) —*

**Document ID:** SRS-001 | **Revision:** 0.3 | **Date:** 2026-04-16 | **Standard:** ISO/IEC/IEEE 29148:2018  
**Tech Stack:** Next.js (App Router) · Supabase (PostgreSQL + Storage + Auth) · Prisma · Gemini API (Vercel AI SDK) · Tailwind CSS · shadcn/ui · pdf-lib · Resend · Vercel  
